...
- Release ID: `record-live2d-motion-2026-07-15.10`
- Profile schema: `live2d-recorder-profile/v1`
- Take contract schema: `live2d-take-contract/v1`
- Package-content-digest: `sha256:0d173d9a6b7625dfac1376016bde3ce2cf7c1f1913c377d198e5274e4dbc2ef6`
...
- 会话级人工许可只确认 `camera-face-tracking` 已启用。当前运行中的有效许可可用于 `yawn`、`idle-soft-loop`、`greet-small/v2`、`sleep-enter/v1`、`happy-small/v2`、`surprised-small/v2` 和 `flustered-small/v2` 的独立手动 take；它不是特定 take 的视觉批准，也绝不启动自动连录。
...
- 对 `happy-small/v2`、`surprised-small/v2` 与 `flustered-small/v2`，VTube Studio 只读参数采样，不得注入 motion、expression 或 hotkey；它们的 CurrentModel 身份和参数签名仍按每个 take 自动重验。
...
2. `happy-small/v2`（`gesture.one-shot`）
3. `surprised-small/v2`（`gesture.one-shot`）
4. `flustered-small/v2`（`gesture.one-shot`）
...
- 录制 `happy-small` revision `v2`：读 [happy-small-v2.md](references/profiles/happy-small-v2.md)，复用有效会话许可但重跑独立自动 preflight；结果只可作为独立 draft。
- 录制 `surprised-small` revision `v2`：读 [surprised-small-v2.md](references/profiles/surprised-small-v2.md)，复用有效会话许可但重跑独立自动 preflight；结果只可作为独立 draft。
- 录制 `flustered-small` revision `v2`：读 [flustered-small-v2.md](references/profiles/flustered-small-v2.md)，复用有效会话许可但重跑独立自动 preflight；结果只可作为独立 draft。
