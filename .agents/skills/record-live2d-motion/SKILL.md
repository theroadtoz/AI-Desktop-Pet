...
- Release ID: `record-live2d-motion-2026-07-15.3`
- Profile schema: `live2d-recorder-profile/v1`
- Take contract schema: `live2d-take-contract/v1`
- Package-content-digest: `sha256:fe7a599a2ff88ad436e67186a894eb6d24af51df87e38a58d2431903aea7fdec`
...
- agent 或自动化在 **human-review** 中只检查技术播放、批准曲线是否存在、参数/通道所有权冲突，以及自然完成或中断后的衔接与恢复；结构、隐私或冲突类硬缺陷仍可阻塞。
- 动作自然度、幅度、节奏观感和美观由用户最终审核；agent 的视觉意见或投票不得覆盖用户结论，也不得仅因审美偏好自行要求重录。
...
8. **parser-readback**：用独立 Motion3 parser 回读 Version 3、Meta、Duration、FPS、Loop、曲线、单调有限数值和 allowlist；结构通过不等于用户视觉通过。
