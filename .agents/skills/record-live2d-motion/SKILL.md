...
description: 受控录制、回读、人工复核和 intake Live2D Motion3 草稿。用于通过 Live2dREC 与 VTube Studio 准备或录制 P2-64Z-2 v2 完整可见组件 profile、检查既有 Motion3、在 Cubism Animator 精修后接入动作；要求完整 41 参数合同、人工视觉审查、兼容性门禁和零越权清理。
---
...
- 仅当 catalog 与 UI 对同一 profile 的 ID、revision=`v2`、status、duration、padding、FPS、Loop、allowlist、requiredOutputParameterIds、outputPrefix 与 profile digest 一致时继续。任何字段缺失、未知、过期或不一致都阻塞；revision 1 不能冒充 v2。
...
- P2-64Z-2 v2 必须绑定完整 41 参数合同：连续集为 `ParamAngleX/Y/Z`、`ParamBodyAngleX/Y/Z`、`ParamEyeLOpen/ROpen`、`ParamEyeLSmile/RSmile`、`ParamEyeBallX/Y`、`ParamBrowLY/BrowLForm`、`ParamMouthForm/MouthOpenY`、`Param28/30`、`Param20/21`、`Param45..Param53`；定时组为 `Param59/60`、`Param67/68/69`、`Param64`、`Param65`、`Param66`、`Param71`、`Param61/62/72`。缺任一参数即在录制前 preflight 阻塞。
- `requiredOutputParameterIds` 必须包含全部 29 个连续参数和全部 12 个定时组件参数；连续参数即使全程平坦也输出明确基线曲线，七个定时组件组即使全程关闭也输出完整成员的全零曲线。
- 连续参数使用普通连续采样/滤波，身体 X/Y/Z 保持连续；定时组件只使用 `0/30` stepped、fade=0、不去尖峰。`eye-symbol` 仅允许 off=`0/0`、heart=`0/30`、star=`30/0`；`held-prop` 的 off/controller/microphone/staff 互斥。
- `Param25/26/27/70` 不重新启用或猜测为泪眼/生气路径；旧路径保持停用。
- 播放期间 motion-owned 参数优先于 expression、physics、pose、breath、accessory 和持久 controller；未参与动作的配件仍由持久 controller 控制。动作完成、中断、超时、失败或 release 后，下一帧恢复最新持久状态，不恢复动作开始快照。
...
- P2-64Z-2 当前阶段在每段 draft 发布后立即隔离展示并由用户人工裁决，不逐段运行额外 post-hoc 正式技术验收。全部 10 段完成并有人工裁决后，才对用户通过的锁定 exact sources 批量运行 post-hoc 技术验收；不得为每段重复代码审查、Skill/profile 批准、全量测试或 agent 审美投票。
...
5. **arm**：对每个手动选择的 profile 自动重跑 preflight，并冻结独立 take contract、当前 profile summary、连接配置、模型身份、能力快照、完整 41 参数 allowlist、requiredOutputParameterIds、时长、padding 和非绝对 outputPrefix；缺任一合同参数、组成员或 required output 立即阻塞，显示不可变 take ID 与短摘要。take TTL 或字段变化只撤销该 take，不撤销会话许可。
6. **countdown**：在仍有效的 armed take 上显示提示音与 `3`、`2`、`1`、`开始`；倒数前再次核对 contract digest，失配即停止。
7. **record**：仅采集当前 profile allowlist；锁定 summary 定义的 capture 时长和 padding，并只写入当前 take ownership ledger 中的 staging candidate。连续集必须保留全平坦曲线，定时组按冻结成员记录完整初始值、切换和最终值；认证/连接失败，或 preflight/录制发现模型身份或参数签名变化时，按 [take-contract.md](references/take-contract.md) 立即撤销会话许可；其他 take 失败只清理该 take。发生参数越界、时长漂移或范围扩大时立即停止，不生成伪成功文件。
8. **parser-readback-and-publish**：用独立 Motion3 parser 回读 Version 3、Meta、冻结的 Duration、FPS、Loop、完整 41 参数曲线和计数；必须验证每个 required output、连续/stepped 插值边界、连续范围、`0/30` 值域、eye-symbol 与 held-prop 合法互斥状态、时间单调、时长和 Meta 计数，以及 motion ownership/恢复证据，不能只比较序列化 JSON 和内存对象。结构通过不等于用户视觉通过。只有 readback 成功后才能原子发布 candidate，并把该输出计为成功 draft；发布后，以及发布前/readback 失败或取消时，均必须重试本 take 临时 candidate 的 unlink，并单独追踪 `cleanupPending`。临时 unlink 持续失败时，必须生成含 `take_id`、owner、受控临时路径、重试状态和禁止删除 final 标记的结构化 cleanup handle，移交 managed drain；发布成功时合法 final 仍为成功 draft，发布前/readback 失败时保留原始录制失败，且 managed drain 不得删除或改报任何合法 final。
9. **isolate-display**：每段完成内建 parser readback、原子发布 draft 和本 take cleanup 后，立即隔离展示给用户并记录人工裁决。旧 `head-pat` 草稿保留原文件及 `visual-retry` 标记，不删除；修复后必须重录。新文件仍先人工审查，不逐段运行额外 post-hoc 正式技术验收。
10. **batch-post-hoc-technical-acceptance**：仅当 P2-64Z-2 的全部 10 段均已录制并完成用户人工裁决后，锁定用户通过的 exact sources，再批量运行额外 post-hoc 正式技术验收。验收失败只说明结构/契约问题，不推翻用户视觉裁决，也不改变各 take 已完成的内建发布与 cleanup 事实。
...
P2-64Z-2 当前阶段不做 runtime intake 或触发映射；它们属于全部 10 段完成、批量 technical-pass 及后续批准之后的下一阶段 TASK。
