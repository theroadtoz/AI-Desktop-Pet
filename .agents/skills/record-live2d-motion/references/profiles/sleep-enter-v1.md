...
- 可复用当前有效的会话级 `camera-face-tracking` 许可，以同步低头与闭眼时序；不能用 API 可用、参数存在或旧会话取代该人工门禁，也不要求为每个 take 再答 `准备好了`。arm 前仍自动重验 CurrentModel、完整 3 项参数签名与 allowlist、时长、FPS、Loop 和 outputPrefix。
...
- 低头与双眼同步是否自然、闭眼终态的幅度和稳定观感、节奏是否舒适、左右摇摆/反弹/嘴部变化及模型从属表现是否美观，均由用户批量最终审核并逐段决定接受或重录。
