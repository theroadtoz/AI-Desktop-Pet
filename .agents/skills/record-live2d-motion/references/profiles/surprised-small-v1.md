...
- 每个独立 take 在 arm 前必须重新由用户确认 camera face tracking 已启用，并重新确认 CurrentModel 身份和完整 4 项参数签名与当前 take contract 一致；旧 take、旧确认和切换前 profile 的 preflight 不可复用。
...
- 惊讶幅度、节奏、自然度、美观和情绪效果由用户最终审核；agent 不得以审美理由要求重录。
