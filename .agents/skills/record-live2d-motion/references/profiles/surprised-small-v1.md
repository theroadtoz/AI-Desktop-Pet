# Surprised Small Profile V1

| 字段 | 固定值 |
| --- | --- |
| `id` / `revision` / `status` | `surprised-small` / `v1` / `ready` |
| `label` / `templateKind` | `轻量惊讶` / `gesture.one-shot` |
| `fps` / `draftLoop` | `30` / `false` |
| `capture_duration` | 录制器 UI 固定 `1.2s` |
| `padding` / `motion_duration` | start `0.2s` + end `0.2s` / `1.6s` |
| `cycle_duration` | 不适用 |
...
- 主体采集窗口为 `1.2s`；导出 Motion3 时间整体加上 `0.2s` leading padding。
...
- UI 录制时长必须为 `1.2s`，固定保留前后各 `0.2s` padding；预期 Motion3 `Meta.Duration=1.6s`、`FPS=30`、`Loop=false`。
