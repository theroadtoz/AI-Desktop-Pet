...
| 字段 | 固定值 |
| --- | --- |
| `id` / `revision` / `status` | `flustered-small` / `v2` / `ready` |
| `label` / `templateKind` | `轻量窘迫` / `gesture.one-shot` |
| `fps` / `draftLoop` | `30` / `false` |
| `capture_duration` | 录制器 UI 固定 `2.3s` |
| `padding` / `motion_duration` | start `0.2s` + end `0.2s` / `2.7s` |
| `cycle_duration` | 不适用 |
...
- 主体采集窗口为 `2.3s`；导出 Motion3 时间整体加上 `0.2s` leading padding。
...
- UI 录制时长必须为 `2.3s`，固定保留前后各 `0.2s` padding；预期 Motion3 `Meta.Duration=2.7s`、`FPS=30`、`Loop=false`。
...
- 窘迫幅度、节奏、自然度、美观和情绪效果由用户批量最终审核；agent 不得重复代码/Skill/profile 审核、进行审美投票或以审美理由要求重录。
