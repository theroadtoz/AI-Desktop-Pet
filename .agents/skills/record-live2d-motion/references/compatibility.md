...
- 两个副本必须同为 release `record-live2d-motion-2026-07-15.9`、profile schema `live2d-recorder-profile/v1` 和 take contract schema `live2d-take-contract/v1`。
...
- 只可复用当前 Live2dREC main service 运行内的 `camera-face-tracking` 人工许可；它覆盖 `yawn`、`idle-soft-loop`、`greet-small/v2`、`sleep-enter/v1`、`happy-small/v1`、`surprised-small/v1` 和 `flustered-small/v1`，但不覆盖任何具体 take 的视觉结论，也不允许自动连录。
