...
test("prompt template: local small model keeps system order but uses shorter first layers", () => {
  ...
  assert.match(cloud[1]?.content ?? "", /(?:事实、技术和安全|技术安全).*(?:直接准确|直答)/);
  ...
  assert.match(local[1]?.content ?? "", /闲聊=.*我先说感受.*具体回应/);
  assert.match(local[0]?.content ?? "", /技术专名准确/);
  assert.match(local[1]?.content ?? "", /≠技术实现/);
  assert.match(local[1]?.content ?? "", /阅历=.*稳妥判断/);
  ...
  assert.match(local[1]?.content ?? "", /闲聊.*我先说感受.*具体回应.*不复述猜心情.*不客服/);
  assert.match(local[1]?.content ?? "", /陈述≠请求.*禁分析拆解总结任务方案步骤.*禁主动问解决事项/);
  assert.match(local[1]?.content ?? "", /魔女视角=.*学院现代魔导.*相关才带入/);
...
test("prompt template: local semantic hints depend only on the latest user question", () => {
  const cases = [
    ...
    {
      content: "今天做什么都提不起劲。",
      pattern: /疲惫陪伴=.*严格2句.*每句≤45字.*首句我.*心疼.*低落.*没精神.*提不起劲.*难熬.*不好受.*次句.*趴着.*安静待.*次句即止.*禁额外段落.*问句.*解释.*建议.*任务/
    },
    {
