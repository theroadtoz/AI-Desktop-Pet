...
const CANDIDATE_BASENAME = "greet-small-20260714-114327-647.motion3.json";
const CANDIDATE_SHA256 = "f456d88757ab7650631364f201fb43812332d903599ef4bc4f751d193ca87bfa";

function makeCurve(id: string, curveIndex: number) {
  const segments: number[] = [0, curveIndex / 100];
  for (let point = 1; point <= 62; point += 1) {
    segments.push(0, (2.4 * point) / 62, curveIndex / 100 + point / 1_000);
  }
...
function makeCandidateMotion() {
  return {
    ...
    Meta: {
      ...
      CurveCount: 10,
      TotalSegmentCount: 620,
      TotalPointCount: 630,
      UserDataCount: 0,
      TotalUserDataSize: 0
    },
    Curves: GREET_SMALL_ALLOWLIST.map(makeCurve),
    UserData: []
  };
}
...
function frameIndex(fps: number) {
  ...
  const frameCount = Math.ceil(3_500 / intervalMs) + 1;
...
function contactFrames() {
  return Array.from({ length: 36 }, (_, frameNumber) => {
...
test("candidate identity pins basename, SHA-256, and bytes independently", () => {
  ...
  assert.deepEqual(validateCandidateIdentity({
    basename: "greet-small-old.motion3.json",
    sha256: "0".repeat(64),
    byteLength: 1
  }).blockers, [
...
test("Motion3 gate accepts exactly 10 approved curves and rejects timing or ownership drift", () => {
  ...
  const wrongDuration = structuredClone(motion);
  wrongDuration.Meta.Duration = 2.5;
  assert.match(validateCandidateMotion(wrongDuration, GREET_SMALL_ALLOWLIST).blockers?.join(",") ?? "", /unexpected-duration/u);

  const oldTakeShape = structuredClone(motion);
  oldTakeShape.Curves.pop();
  oldTakeShape.Meta.CurveCount = 9;
  oldTakeShape.Meta.TotalSegmentCount = 558;
  oldTakeShape.Meta.TotalPointCount = 567;
  assert.match(validateCandidateMotion(oldTakeShape, GREET_SMALL_ALLOWLIST).blockers?.join(",") ?? "", /allowlist-id-not-present/u);
...
test("lifecycle requires exactly queued -> started -> completed -> restore_started -> restore_completed", () => {
  const good = summarizeLifecycle([
    event("queued", 10),
    event("started", 20),
    event("completed", 2_420),
    event("restore_started", 2_421),
    event("restore_completed", 2_430)
  ], "run-1");
  ...
  assert.equal(summarizeLifecycle([
    ...
    event("started", 20),
    event("restore_started", 2_421),
    event("completed", 2_422),
    event("restore_completed", 2_430)
  ], "run-1").passed, false);
  assert.equal(summarizeLifecycle([
    event("queued", 10), event("started", 20), event("completed", 2_420),
    event("watchdog_fired", 2_421), event("stop_all_motions", 2_422),
    event("restore_started", 2_423), event("restore_completed", 2_430)
  ], "run-1").passed, false);
  const stopAfterCompletion = summarizeLifecycle([
    event("queued", 10), event("started", 20), event("completed", 2_420),
    event("restore_started", 2_421), event("restore_completed", 2_430),
    event("stop_all_motions", 2_431, { callIndex: 3 })
  ], "run-1");
...
test("contact sheets cover every selected frame and enlarge both high-risk windows", () => {
  ...
  try {
    ...
    assert.equal(index.riskCoverage["1400-1830"].length, 5);
    assert.equal(index.riskCoverage["2100-2200"].length, 2);
    assert.equal(index.sheets.filter(({ kind }) => kind === "overview").length, 2);
    assert.equal(index.sheets.some(({ filename }) => filename.startsWith("contact-sheet-risk-1400-1830-")), true);
    assert.equal(index.sheets.some(({ filename }) => filename.startsWith("contact-sheet-risk-2100-2200-")), true);
