...
} from "./p2-20i-stage-offline-local-llm-install-layout.mjs";
...
const keepTmpEnv = "P2_20I_KEEP_TMP";
const questions = [
  "用一句话回答：现在是哪一天？",
...
export function shouldKeepP2_20IStaging(env = process.env) {
  return env[keepTmpEnv] === "1";
}

export function cleanupP2_20IStagingOnSuccess(stagingRoot, env = process.env) {
  if (shouldKeepP2_20IStaging(env)) {
    return {
      cleanupStatus: "kept"
    };
  }
...
async function main() {
  ...
  try {
    if (!existsSync(paths.localLlmRoot)) {
      const stageResult = await stageOfflineLocalLlmInstallLayout({ repoRoot });
      ...
      if (!stageResult.ok) {
        printSummary({
          ...
          status: "blocked",
          runtime: runtimeName,
          reason: "install_layout_stage_failed",
    ...
    const resolved = modules.resolveBundledLlamaCppRuntime({
      env: {},
      cwd: join(paths.stagingRoot, "unrelated-cwd"),
    ...
    let cleanup = { cleanupStatus: "skipped" };
    ...
    if (ok) {
      cleanup = cleanupP2_20IStagingOnSuccess(paths.stagingRoot);
    }
