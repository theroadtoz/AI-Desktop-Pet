...
export const stagingRootName = "p2-20j-extra-resources";
export const packageOutputRootName = "p2-20j-package-output";
...
export function getP2_20JPaths(repoRoot = getRepoRoot()) {
  const tmpRoot = resolve(repoRoot, ".tmp");
  const stagingRoot = join(tmpRoot, stagingRootName);
  const stagedLocalLlmRoot = join(stagingRoot, "local-llm");
  const packageOutputRoot = join(tmpRoot, packageOutputRootName);
...
export function assertSafeP2_20JTmpRoot(candidateRoot, repoRoot = getRepoRoot()) {
  const tmpRoot = resolve(repoRoot, ".tmp");
  ...
  if (resolvedRoot !== tmpRoot && !resolvedRoot.startsWith(tmpPrefix)) {
    throw new Error("p2_20j_destination_outside_repo_tmp");
  }
}

export async function stageElectronBuilderLocalLlmExtraResources(options = {}) {
  ...
  assertSafeP2_20JTmpRoot(paths.stagingRoot, repoRoot);
  ...
  mkdirSync(dirname(paths.stagedLocalLlmRoot), { recursive: true });
  cpSync(source.root, paths.stagedLocalLlmRoot, {
    recursive: true,
    force: true,
    errorOnExist: false
  });

  const fileCounts = countFiles(paths.stagedLocalLlmRoot);
  ...
  return {
    ...
    summary: removeUndefined({
      ...
      sourceRootName: basename(source.root),
      destinationRootName: basename(paths.stagingRoot),
      destinationLocalLlmName: basename(paths.stagedLocalLlmRoot),
  ...
  stageElectronBuilderLocalLlmExtraResources()
    ...
    .catch((error) => {
      printSummary({
        ...
        runtime: runtimeName,
        safeSummaryOnly: true,
        reason: error instanceof Error && error.message === "p2_20j_destination_outside_repo_tmp"
          ? error.message
