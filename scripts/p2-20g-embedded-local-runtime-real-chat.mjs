...
const envRootName = "AI_DESKTOP_PET_BUNDLED_LLAMA_CPP_ROOT";
...
const systemMessage = {
  role: "system",
  content: "你是 Windows Live2D AI 桌宠里的老魔女角色，掌握现代科技、耐心、乐观、学识渊博。当前日期是 2026-07-02，星期四。回答要简短、亲切，并直接对应用户问题。"
};
...
async function main() {
  ...
  if (!stagingRoot) {
    printSummary({
      ...
      status: "blocked",
      runtime: runtimeName,
      reason: "missing_bundled_staging_root",
      resourceRootName: ".tmp"
    });
    return;
  }

  const resolved = resolveBundledLlamaCppRuntime({
    env: { [envRootName]: stagingRoot },
    cwd: process.cwd(),
    resourcesPath: ""
  });
...
function evaluateRoundRelevance(roundIndex, reply, replyFingerprints) {
  ...
  return normalized.length >= 2 ? "matched" : "too_short";
}
...
function resolveStagingRoot() {
  const envRoot = readNonEmpty(process.env[envRootName]);
  ...
  const candidates = [
    join(process.cwd(), ".tmp", "p2-20g-embedded-local-llm", "local-llm"),
    join(process.cwd(), ".tmp", "p2-20g-embedded-local-llm"),
    join(process.cwd(), ".tmp", "p2-20g-local-llm"),
    join(process.cwd(), ".tmp", "local-llm")
  ];
